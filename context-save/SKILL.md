...
Read `context.md` frontmatter only (`grep -m1` per field). Build
...
if [ -n "$PARENT_SNAPSHOT_PATH" ]; then
  ...
  P_DEC=$(grep -c '^- ' "$DIR/DECISIONS.md" 2>/dev/null||echo 0); N_DEC=$(grep -c '^- ' "$TMP_FOLDER/DECISIONS.md"||echo 0)
  P_PROG=$(grep -c '^- ' "$DIR/PROGRESS.md"||echo 0);  N_PROG=$(grep -c '^- ' "$TMP_FOLDER/PROGRESS.md"||echo 0)
  P_RES=$(grep -c '^### ' "$DIR/RESULTS.md"||echo 0);   N_RES=$(grep -c '^### ' "$TMP_FOLDER/RESULTS.md"||echo 0)

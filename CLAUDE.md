...
Newest bullets first. Compact format — one-line rule plus `❌ Bad` / `✅ Good` sub-bullets. Capture lessons manually as they surface in conversation (the prior `pattern-observer` auto-capture hook was removed). Examples must be generic (no project codenames, ticket IDs, internal artifacts). If a lesson is genuinely agent-neutral (no `~/.claude/*` paths, no Claude-specific models or tools, no `Agent`-tool semantics), promote it to `AGENTS.md` Lessons instead.

- **Mirror personal skills + hooks across all agent dirs** — When installing or removing a personal skill or hook, symlink (or remove) it in every agent's surface in the same turn. Skills: Claude Code at `~/.claude/skills/`, Codex at `~/.agents/skills/`. Hooks: Claude Code script at `~/.claude/hooks/` + entry in `~/.claude/settings.json`; Codex script at `~/.codex/hooks/` + entry in `~/.codex/hooks.json`. The script file is the same source (one canonical copy under `biot-awesome-skills/hooks/`); the wiring is necessarily per-agent because settings shape differs. Codex tracks a `trusted_hash` per hook entry in `~/.codex/config.toml`; editing `hooks.json` invalidates it and codex will re-prompt on next launch — surface this once when wiring.
  - ❌ Bad: Symlink `<repo>/hooks/<hook>.sh` into `~/.claude/hooks/`, add entry to `~/.claude/settings.json`, call it done — codex never sees the hook.
  - ✅ Good: Symlink into both `~/.claude/hooks/` and `~/.codex/hooks/`, add wiring entries to both `~/.claude/settings.json` and `~/.codex/hooks.json`, warn the user about codex's trust-hash re-prompt.

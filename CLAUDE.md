...
`/context-save-rolling` **before your final response**. The policy is
...
The legacy gstack `/context-save` and v2 rolling `CURRENT-<topic>.md`
files are still readable by restore as fallbacks; new saves always
write the v3 folder layout. Legacy files are never deleted by the
save flow.

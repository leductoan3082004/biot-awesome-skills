...
- **Code-walk default shape: prune + inline-comment + branch-name, no post-block re-narration** — When the user signals "walk me through", "explain how X works", "trace the flow", or "I won't open my IDE", default response is: name the branches/control-flow in 1–2 sentences ABOVE each block, prune the snippet to only the lines that carry the explanation (replace skipped regions with `// ...` or the language's comment syntax), place inline comments on each non-trivial line explaining WHAT and WHY, recursively inline every in-codebase callee until a framework/stdlib leaf, and **never** follow the block with a numbered list that re-describes the same lines. Post-block prose is reserved for gotchas, edge cases, dormant paths, and bridges to the next callee — content NOT already in the snippet.
  - ❌ Bad: Paste a verbatim 30-line function, then below it write "1. Validates X. 2. Calls Y. 3. Returns Z." re-narrating lines already in the block.
  - ✅ Good: Name the branches above, paste a pruned ~8-line excerpt with inline `// comment` on each non-trivial line; reserve post-block prose for insights not visible in the snippet.

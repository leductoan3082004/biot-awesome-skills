...
- **Skill upgrades with on-disk state must verify backward-compat with a load test, not just spec text** — When upgrading a skill that has persisted data from a prior version, the new skill must (a) read prior-version artifacts as parents without modifying them, and (b) be load-tested against those legacy artifacts before declaring done. A "legacy fallback" sentence in the SKILL.md is necessary but not sufficient — run a sub-agent through the new skill with a prior-version fixture in place and verify the legacy file is byte-identical after the upgrade migration.
  - ❌ Bad: "New `<some-skill>` reads old `<legacy-file>` as parent — see SKILL.md." → ship without testing → user discovers the migration silently overwrites the legacy file on first run.
  - ✅ Good: Spin up a fixture containing the legacy artifact, dispatch a sub-agent through the upgraded skill, `cmp`/`diff` the legacy artifact before vs after, confirm byte-identity AND that the new artifact carries the legacy content forward correctly.
